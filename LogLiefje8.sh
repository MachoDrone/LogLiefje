...
TEMP_ADDITIONAL="additional_data.log"

cleanup() {
    rm -f "$RAW_LOG_FILE" "$NUMBERED_LOG_FILE" "$FILTERED_LOG_FILE" "$TMP_UPLOAD_LOG" "$TEMP_ADDITIONAL"
}
...
docker logs -t --tail 1000000 nosana-node | \
awk '{print NR, $0; if (NR % 10000 == 0) { printf("\rLines processed: %d", NR) > "/dev/stderr" }} END { print "" > "/dev/stderr" }' > "$RAW_LOG_FILE"
...
echo "Collecting host wallet addresses from logs. Please wait..."
wallet_line=$(docker logs -t nosana-node 2>&1 & pid=$! ; while read -r line; do if echo "$line" | grep -q 'Wallet:' ; then echo "$line" ; kill $pid ; break ; fi ; done)
wallet_addr=$(echo "$wallet_line" | awk '{print $3}' | sed -r 's/\x1B\[[0-9;]*[mK]//g')
host_addrs=( "$wallet_addr" )
...
echo "Assembling additional data..."
{
...
cat "$TEMP_ADDITIONAL" > "$COMBINED_LOG_FILE"
cat "$FILTERED_LOG_FILE" >> "$COMBINED_LOG_FILE"
