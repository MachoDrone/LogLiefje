...
RAW_LOG_FILE="lastNhours.log"
NUMBERED_LOG_FILE="lastNhours_numbered.log"
TMP_UPLOAD_LOG="/tmp/0x0_upload.log"
COMBINED_LOG_FILE="combined_log.log"

cleanup() {
    rm -f "$RAW_LOG_FILE" "$NUMBERED_LOG_FILE" "$TMP_UPLOAD_LOG" "$COMBINED_LOG_FILE"
}
...
echo "Extracting last $HOURS hour(s) of logs with live line counter. Please wait..."
docker logs --timestamps --since ${HOURS}h nosana-node | \
...
echo "Collecting additional system and log data. Please wait, this may take some time depending on log size..."

discord_user_sanitized=$(echo "$discord_user" | tr -c 'A-Za-z0-9_-' '_')
timestamp=$(date -u +%Y%m%dT%H%M%SZ)
logfile="${timestamp}_${discord_user_sanitized}.log"

echo "Collecting host wallet addresses. Please wait..."
mapfile -t host_addrs < <(
  docker logs nosana-node | grep 'Wallet:' | awk '{print $3}' | \
...
print_host_line() {
  ...
  if [ -n "$addr" ]; then
    ...
    echo "Host: Wallet not found"
  fi
}

echo "Detecting running environment. Please wait..."
...
echo "Searching for API offline events in last 24 hours. Please wait..."
...
echo "Collecting docker container status. Please wait..."
...
echo "Collecting podman container status and frpc-api logs. Please wait..."
if [ -f /proc/version ] && grep -qi "Microsoft" /proc/version; then
...
echo "Collecting system info. Please wait..."
uname_a=$(uname -a)

echo "Combining collected data into log file. Please wait..."
{
  ...
  print_host_line "${host_addrs[0]}"
  echo
  echo "API offline events and context (last 24hr):"
  if [ -z "$api_event_logs" ]; then
    echo "No API offline events found in the last 24 hours."
  else
    echo "$api_event_logs"
  fi
  echo
  echo "Full numbered docker logs (last $HOURS hour(s)):"
  cat "$NUMBERED_LOG_FILE"
  echo
} > "$COMBINED_LOG_FILE"

filesize=$(ls -lh "$COMBINED_LOG_FILE" | awk '{print $5}')
tmpfile=$(mktemp)
echo "File: $COMBINED_LOG_FILE  Size: $filesize" > "$tmpfile"
cat "$COMBINED_LOG_FILE" >> "$tmpfile"
mv "$tmpfile" "$COMBINED_LOG_FILE"

LOG_FILE_PATH="$COMBINED_LOG_FILE"

# ------------- UPLOAD TO 0x0.st WITH PROGRESS BAR -------------
echo "Uploading file size: $(du -h "$LOG_FILE_PATH" | cut -f1)"
echo "Uploading $LOG_FILE_PATH to 0x0.st. Please wait..."
