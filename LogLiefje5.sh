...
cleanup() {
    rm -f "$RAW_LOG_FILE" "$NUMBERED_LOG_FILE" "$FILTERED_LOG_FILE" "$TMP_UPLOAD_LOG" "$COMBINED_LOG_FILE"
}
...
echo "Collecting additional system and log data. This may take some time depending on log size..."

discord_user_sanitized=$(echo "$discord_user" | tr -c 'A-Za-z0-9_-' '_')
timestamp=$(date -u +%Y%m%dT%H%M%SZ)
logfile="${timestamp}_${discord_user_sanitized}.log"

echo "Collecting host wallet addresses from all logs..."
...
echo "Combining collected data into log file..."
{
