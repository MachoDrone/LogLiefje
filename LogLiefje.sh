#!/bin/bash
echo "v0.01" #update after each edit
sleep 2 #short sleep for viewing
...
Usage: $0 [options]
Options:
  -u, --user HQs user ID to notify (e.g., U06QUUFGGA8)
  -c, --channel HQs channel ID or user ID for DM (default: DM to user)
  ...
  --profile Use a profile-specific config file (e.g., config..txt)
  -h, --hours Number of hours to go back in logs (1-24, default: 24)
...
# ------------- DEFAULTS WHEN NO ARGS PASSED -------------
if [[ $ORIG_ARGC -eq 0 && -z "$USER_ID" && -z "$CHANNEL_ID" ]]; then
    USER_ID="C09AX202QD7"
...
if [[ -z "$CHANNEL_ID" ]]; then
    if [[ -n "$USER_ID" ]]; then
        ...
        echo -e "Use -c or -u or enable config with -e."
        echo -e "******************************************************"
        exit 1
    fi
fi
...
' "$NUMBERED_LOG_FILE" > "$FILTERED_LOG_FILE.tmp"

# ------------- DATA COLLECTION FROM SCRIPT 2 -------------
REQUIRE_DISCORD_USERNAME=1
if [ "${REQUIRE_DISCORD_USERNAME:-1}" -eq 1 ]; then
    while true; do
        read -p "v6.7.814 Please add your Discord username: " discord_user
...
wallet_addr=$(
  docker logs -t nosana-node 2>/dev/null |
  sed -r 's/\x1B\[[0-9;]*[mK]//g' |
  grep -m1 -E 'Wallet:[[:space:]]*[1-9A-HJ-NP-Za-km-z]{32,48}' |
  sed -E 's/.*Wallet:[[:space:]]*([1-9A-HJ-NP-Za-km-z]{32,48}).*/\1/'
)

host_addrs=()
host_addrs[0]="$wallet_addr"

print_host_line() {
  local addr="$1"
  if [ -n "$addr" ]; then
    echo "Host: http://dashboard.nosana.com/host/$addr"
  else
    echo "Host: "
  fi
}
...
echo "Appending collected data to log file..."
{
  ...
  echo "$environment"
  date -u
  echo "Collection of logs for Low Availability or API offline"
  echo
  echo "docker ps:"
  echo "$docker_ps"
  echo
...
cat "$FILTERED_LOG_FILE.tmp" >> "$FILTERED_LOG_FILE"
...
UPLOAD_RESPONSE=$(curl -# \
    -A "LogLiefje/1.0" \
    -F "file=@$LOG_FILE_PATH" https://0x0.st 2>&1 | tee "$TMP_UPLOAD_LOG")
...
$discord_user
http://dashboard.nosana.com/host/${wallet_addr}
\`\`\`wget $UPLOAD_URL && cat $BASENAME\`\`\`
