#!/bin/bash
# nosana-logs.sh v0.01.7
# Reliable log viewer for Nosana nodes running inside nested Podman-in-Docker.
# Reads log files directly — bypasses `podman logs` (which hangs on large logs).
# Auto-detects both outer Docker and inner Podman containers by image name.
#
# Usage:
#   ./nosana-logs.sh              # show full log from all detected containers
#   ./nosana-logs.sh --tail 100   # last 100 lines
#   ./nosana-logs.sh --head 50    # first 50 lines
#   ./nosana-logs.sh -g "error"   # grep for pattern (extended regex)
#   ./nosana-logs.sh -f           # follow mode (like tail -f)
#   ./nosana-logs.sh -c podman-gpu0  # target a specific outer Docker container
#   ./nosana-logs.sh -i nosana-node  # target a specific inner Podman container
#   ./nosana-logs.sh -o           # write combined log: {hostname}-all-nosana-node.log
#   ./nosana-logs.sh -o --uncombine  # write separate files per container
...
# Image names used for auto-detection
OUTER_IMAGE="nosana/podman"
INNER_IMAGE="nosana/nosana-node"

usage() {
    cat <<EOF
nosana-logs.sh v${VERSION}
Tail nosana-node logs directly from the log file (no podman logs hang).
...
Auto-detection:
  ...
  Inner: finds Podman containers running the '$INNER_IMAGE' image
EOF
    exit 0
}
...
while getopts "fg:c:i:ohv" opt; do
    case "$opt" in
        ...
        o) OUTPUT_FILE=true ;;
        h) usage ;;
        v) echo "nosana-logs.sh v${VERSION}"; exit 0 ;;
        *) usage ;;
    esac
done
...
for CONTAINER in "${CONTAINERS[@]}"; do
    if [[ -n "$INNER_CONTAINER" ]]; then
        ...
        mapfile -t INNER_NAMES < <(docker exec "$CONTAINER" podman ps --format '{{.Names}}\t{{.Image}}' 2>/dev/null | grep "$INNER_IMAGE" | awk '{print $1}')
        if [[ ${#INNER_NAMES[@]} -eq 0 ]]; then
            echo "WARNING: No '$INNER_IMAGE' container found inside '$CONTAINER'. Skipping." >&2
            continue
        fi
    fi
...
if [[ ${#PAIRS[@]} -eq 0 ]]; then
    echo "ERROR: No nosana-node log paths found." >&2
    exit 1
fi
...
if [[ "$OUTPUT_FILE" == true && "$COMBINE" == true ]]; then
    if [[ ${#PAIRS[@]} -eq 1 ]]; then
        ...
        COMBINEFILE="${HOSTNAME}-all-nosana-node.log"
    fi
    > "$COMBINEFILE"
    COMBINE_FIRST=true
fi
