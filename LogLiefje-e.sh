...
TMP_ADDITIONAL_LOG="/tmp/additional.log"
cleanup() {
    rm -f "$RAW_LOG_FILE" "$NUMBERED_LOG_FILE" "$FILTERED_LOG_FILE" "$TMP_UPLOAD_LOG" "$TMP_ADDITIONAL_LOG"
}
...
if [ "${REQUIRE_DISCORD_USERNAME:-1}" -eq 1 ]; then
  while true; do
    echo -en "${YELLOW}v6.7.814 Please add your Discord username: ${NC}"
    read -r discord_user </dev/tty
...
cat "$TMP_ADDITIONAL_LOG" > "$FILTERED_LOG_FILE.tmp"
echo "Full Docker logs (last $HOURS hours):" >> "$FILTERED_LOG_FILE.tmp"
cat "$FILTERED_LOG_FILE" >> "$FILTERED_LOG_FILE.tmp"
mv "$FILTERED_LOG_FILE.tmp" "$FILTERED_LOG_FILE"
