#!/bin/bash
# job-logs.sh v0.00.2
# Log viewer for job containers running inside nested Podman-in-Docker.
# Reads log files directly — bypasses `podman logs` (which hangs on large logs).
# Auto-detects both outer Docker and inner Podman containers.
# Job containers are identified by exclusion: anything NOT nosana-node and NOT frpc-*.
#
# Usage:
#   ./job-logs.sh              # show full log from all detected job containers
#   ./job-logs.sh --tail 100   # last 100 lines
#   ./job-logs.sh --head 50    # first 50 lines
#   ./job-logs.sh -g "error"   # grep for pattern (extended regex)
#   ./job-logs.sh -f           # follow mode (like tail -f)
#   ./job-logs.sh -c podman-gpu0  # target a specific outer Docker container
#   ./job-logs.sh -i 7SKsU     # target a specific job container (partial match)
#   ./job-logs.sh -o           # write combined log: {hostname}-all-jobs.log
#   ./job-logs.sh -o --uncombine  # write separate files per container
...
EXCLUDE_NAMES='^(nosana-node|frpc-)'

usage() {
    cat <<EOF
job-logs.sh v${VERSION}
Tail job container logs directly from the log file (no podman logs hang).
Job containers are detected by exclusion: anything NOT nosana-node or frpc-*.
...
Options:
  ...
  -i NAME   Target a specific inner job container (supports partial match)
  -o        Write output to file (combined; uses -all- only for multi-container hosts)
  --uncombine  With -o, write separate files: {hostname}-{docker}-{inner}.log
  -h        Show this help
  -v        Show version

Auto-detection:
  Outer: finds Docker containers running the '$OUTER_IMAGE' image
  Inner: finds Podman containers that are NOT nosana-node or frpc-*
EOF
    exit 0
}
...
while getopts "fg:c:i:ohv" opt; do
    case "$opt" in
        ...
        o) OUTPUT_FILE=true ;;
        h) usage ;;
        v) echo "job-logs.sh v${VERSION}"; exit 0 ;;
        *) usage ;;
    esac
done
...
for CONTAINER in "${CONTAINERS[@]}"; do
    if [[ -n "$INNER_CONTAINER" ]]; then
        # Partial match: find job containers matching the user's filter
        mapfile -t INNER_NAMES < <(docker exec "$CONTAINER" podman ps --format '{{.Names}}' 2>/dev/null | grep -Ev "$EXCLUDE_NAMES" | grep "$INNER_CONTAINER")
    else
        mapfile -t INNER_NAMES < <(docker exec "$CONTAINER" podman ps --format '{{.Names}}' 2>/dev/null | grep -Ev "$EXCLUDE_NAMES")
    fi
    if [[ ${#INNER_NAMES[@]} -eq 0 ]]; then
        echo "WARNING: No job containers found inside '$CONTAINER'. Skipping." >&2
        continue
    fi
...
if [[ ${#PAIRS[@]} -eq 0 ]]; then
    echo "ERROR: No job container log paths found." >&2
    exit 1
fi
...
if [[ "$OUTPUT_FILE" == true && "$COMBINE" == true ]]; then
    if [[ ${#PAIRS[@]} -eq 1 ]]; then
        IFS='|' read -r C I _ <<< "${PAIRS[0]}"
        COMBINEFILE="${HOSTNAME}-${C}-${I}.log"
    else
        COMBINEFILE="${HOSTNAME}-all-jobs.log"
    fi
    > "$COMBINEFILE"
    COMBINE_FIRST=true
fi
...
for PAIR in "${PAIRS[@]}"; do
    ...
    OUTFILE="${HOSTNAME}-${CONTAINER}-${INNER}.log"
    ...
    if [[ "$FOLLOW" == true ]]; then
        ...
        if [[ ${#PAIRS[@]} -gt 1 ]]; then
            echo "=== ${HOSTNAME}-${CONTAINER}-${INNER} ===" >> "$COMBINEFILE"
        fi
        if [[ -n "$GREP_PATTERN" ]]; then
