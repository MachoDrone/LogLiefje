...
if [ -f /proc/version ] && grep -qi "Microsoft" /proc/version; then
  ...
  frpc_api_log=$(podman ps -a --format '{{.Names}}' 2>&1 | grep '^frpc-api-' | xargs -I {} podman logs -t {} 2>&1 | grep -v "command not found")
else
  ...
  frpc_api_log=$(docker exec podman podman ps --format '{{.Names}}' 2>&1 | grep '^frpc-api-' | xargs -I {} docker exec podman podman logs -t {} 2>&1 | grep -v "command not found")
fi
